import { applyBlastImpulse } from './blastPhysics';
...
class PhysicsRuntime {
  ...
  /** Smooth, bounded blast pressure shared by props and fracture debris. */
  applyRadialImpulse(center: Vector3Tuple, radius: number, strength: number, up = 0.4) {
    for (const entry of this.entries.values()) applyBlastImpulse(entry.body, center, radius, strength, up);
  }
