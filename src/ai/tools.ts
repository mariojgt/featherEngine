...
const rawEngineTools = {
  ...
  create_sim_racing_template: tool({
    description:
      'Build the SIM-RACING "Proving Ground" SANDBOX showcase for the REAL (raycast) vehicle physics: a Rapier DynamicRayCastVehicleController car with genuine per-wheel suspension, weight transfer, tire friction and true rollovers, free-roaming a playground of toys — kicker ramp + landing, banked turn, rolling washboard, slalom cones, fracture-able crates/barrels, explosive barrel chains, car football with a scoring goal, a rotating sweeper, a piston gate, an air ring, a domino run, boost pads, light towers, grandstands, dusk sky + bloom. Style scoring runs through a COMBO BANK: drift/air points sit pending in a "Combo" HUD chip, bank into "Score" after a second of clean driving, and are LOST if you crash first. Garage (G), speed setup (P), night/rain toggles (N/M — rain spray + Wet grip). No race/rivals — pure sandbox (wire your own countdown with the engine race-start kit if wanted). Returns the player car objectId.',
    inputSchema: z.object({}),
    execute: async () => {
      const id = await createSimRacingTemplate();
      return id
        ? `Built the Sim Racing sandbox — raycast-sim car objectId ${id}. Press Play and free-roam the proving ground: W/S throttle+brake, A/D steer, Space handbrake, Shift nitro, E/Q gears, C camera, G garage, P speed setup, N night, M rain. Drift/air style points bank through the COMBO chip (crash before banking and they're lost). Real suspension works the ramps and rollers, and a curb or hard hit can roll the car. Tune it in the Inspector (Physics Model = Raycast Sim) or via set_vehicle.`
