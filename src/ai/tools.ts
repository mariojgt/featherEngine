...
const rawEngineTools = {
  ...
  set_character_controller: tool({
    ...
    inputSchema: z.object({
      ...
      jumpBufferTime: z.number().optional().describe('Jump pressed up to this many seconds BEFORE landing fires on touchdown. Default 0.15. 0 disables.'),
      landingRecovery: z.number().optional().describe('0..1: hard landings briefly sap speed + dip the camera, scaled by impact (drives the "landing" animator source). Default 0.4. 0 disables.'),
      apexHang: z.number().optional().describe('Gravity ×multiplier near the jump apex — <1 adds a brief controllable hang at the peak. Default 0.65. 1 disables.'),
      sprintTurnFactor: z.number().optional().describe('Turn-rate ×multiplier at full sprint (full rate at a walk) so fast runs carve wider arcs. Default 0.55. 1 disables.'),
      slideEnabled: z.boolean().optional().describe('Sprint-slide: tapping crouch at sprint speed drops into a steerable momentum slide (drives the "sliding" animator source; jump cancels into a slide-hop). Default true.'),
      slideDuration: z.number().optional().describe('Max slide seconds. Default 0.9.'),
      slideSpeedBoost: z.number().optional().describe('Slide entry speed = current speed × this. Default 1.2.'),
      ...
      // Lock-on targeting (Z-targeting). Targets = objects with a `health` instance variable > 0 or an `enemy` tag.
      lockOnEnabled: z.boolean().optional().describe('Lock-on targeting: keyLockOn toggles a lock on the nearest living target (object with health > 0 or enemy tag); the character strafes facing it and the camera frames both. Default true.'),
      keyLockOn: z.string().optional().describe('Lock-on toggle key. Default "KeyT".'),
      lockOnRange: z.number().optional().describe('Max distance to acquire a lock-on target. Default 16.'),
      lockOnBreakDistance: z.number().optional().describe('Distance at which an existing lock breaks. Default 22.'),
      keySwapShoulder: z.string().optional().describe('Tap to mirror the third-person camera to the other shoulder (also flips the aim shoulder shift). Default "KeyV".'),
  ...
  create_third_person_template: tool({
    description:
      "Build a complete Unreal-style THIRD-PERSON TUTORIAL CORRIDOR — six connected rooms (each a one-system lesson) walked in order along +Z, linked by colour-coded door portals with jamb glows and thresholds: ROOM 01 MOVEMENT (sprint/vault lanes, vaultable rails, a jump pad: Trigger Enter → Apply Impulse), ROOM 02 RAGDOLL ([E] pedestal hands the character to physics, P recovers), ROOM 03 WATER (a real Water Volume flips the controller into swim mode), ROOM 04 CLIMB (climb-volume trigger + rung wall up to an outlook), ROOM 05 INTERACTION (a light theatre: statue lit by 3 coloured lights, each on an [E] toggle pedestal driving Set Active), ROOM 06 CINEMATIC (a gold slate / [E] pedestal that plays a real Film Mode camera pass with grade + letterbox). Every room has a lesson pad that updates a top-right TUTORIAL COACH panel (ROOM X / 06 progress chip + title/body/status) and writes the player's checkpoint vars. Rendering ships polished: bright UE-daylight procedural sky, subtle volumetric haze with sun shafts, High quality preset, a faint filmic color grade, bloom + vignette, per-room accent lighting, ceiling beams/skirting/wall caps so rooms read as built interiors. Spawns the bundled player rig with the Fist/Bat/Pistol inventory, health/interaction/ranged kits, and an AAA-tuned controller (centred behind-the-character cam, smooth accel/decel, soft turn, strafe off, mantle/vault). HUD: glass stats panel with bound health bar, centred bottom controls pill, world-space welcome billboard. Returns pawn objectId.",
    inputSchema: z.object({}),
    execute: async () => {
  ...
  create_sim_racing_template: tool({
    description:
      'Build the SIM-RACING "Proving Ground" showcase for the REAL (raycast) vehicle physics: a Rapier DynamicRayCastVehicleController car with genuine per-wheel suspension, weight transfer, tire friction and true rollovers — PLUS a full RACE: a neon perimeter circuit of "Checkpoint <n>" gates (the engine lap timer + AI driving line read the same gates), THREE AI rival cars on a staggered grid (aiDriver autopilot with rubber-banding), a 3-2-1-GO countdown blueprint that holds the grid via the Driving var, a physical START-LIGHT TREE gantry (lamps named "Start Light 1..3" follow the Count var — red steps, green GO, synthesized beeps), a PERFECT-LAUNCH bonus (full throttle at the green = free Nitro + banner), and a race HUD (position/lap/best + countdown banner + slipstream chip). Style scoring runs through a COMBO BANK: drift/air points sit pending in a "Combo" HUD chip, bank into "Score" after a second of clean driving, and are LOST if you crash first. The infield keeps the proving-ground toys: kicker ramp, banked turn, slalom cones, fracture crates/barrels, car football, domino run, boost pads, garage (G), speed setup (P), night/rain toggles (N/M — rain spray + Wet grip). Returns the player car objectId.',
    inputSchema: z.object({}),
    execute: async () => {
      const id = await createSimRacingTemplate();
      return id
        ? `Built the Sim Racing template — raycast-sim player car objectId ${id}, on pole ahead of three AI rivals. Press Play: the start-light tree counts down 3-2-1 (beeps; full throttle at the green = PERFECT LAUNCH nitro bonus), then race the neon circuit — lap timer, live position, slipstream, and a drift/air COMBO bank that you lose by crashing. W/S throttle+brake, A/D steer, Space handbrake, Shift nitro, E/Q gears, C camera, G garage, P speed setup, N night, M rain. Tune the car in the Inspector (Physics Model = Raycast Sim) or via set_vehicle.`
  ...
  add_node: tool({
    ...
    inputSchema: z.object({
      ...
      eventName: z.string().optional(),
      functionName: z.string().optional().describe('Function / Call Function: binds the call to its entry by name.'),
      switchCases: z.array(z.string()).optional().describe("Switch: the case labels matched against the stringified Value; each case gets exec handle 'case-0','case-1',… and no-match fires the default 'exec-out'."),
      roundMode: z.enum(['round', 'floor', 'ceil']).optional().describe('Round node mode.'),
