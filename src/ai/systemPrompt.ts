...
- **Game quality / scalability (set_quality):** an Unreal-style project-wide QUALITY preset — Low / Medium / High / Epic — in renderSettings.quality. It scales render resolution (DPR), shadow casting + shadow-map size, post-FX MSAA, and bloom blur, applying live in the editor viewport, Play, and export. Use **Low** when the user reports lag/low FPS or is on weak hardware (no shadows, 0.75× res), **Epic** for screenshots/showcase (4× MSAA, 2× res, big shadow budget); High is the balanced default. It's also a viewport dropdown and the **"Set Quality"** Blueprint node (qualityLevel) — wire that node to a settings-menu button or custom event so players can change quality in-game.
