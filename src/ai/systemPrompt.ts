    ...
    ...(detail === 'tiny'
      ? {
          ...
          vehicle: object.vehicle?.enabled
            ? { maxSpeed: object.vehicle.maxSpeed, cameraFollow: object.vehicle.cameraFollow, wheels: object.vehicle.wheelObjectIds.length }
            : null,
      ...
      : {
          ...
          vehicle: object.vehicle?.enabled
            ? {
                ...
                turnRate: object.vehicle.turnRate,
...
- **Vehicle / driving (set_vehicle):** the built-in arcade CAR controller, the driving peer of the character controller. WASD drives — W throttle, S brake/reverse, A/D steer, Space handbrake, H horn — with a signed forward speed (accel/braking/drag), speed-scaled steering (no turning in place), and a top speed. The car body should be a **dynamic Rapier body** (use collider "convex" so it matches an imported car model); the vehicle pass drives its horizontal motion + yaw while **Rapier owns vertical** — so the car rides terrain, climbs ramps, and bumps props for free. **Suspension is a visual feel** (tunable): the chassis squats/dives (bodyPitch) under accel/brake and leans into turns (bodyRoll), settling at suspensionStiffness; the **wheel child objects spin** (∝ speed/wheelRadius) and the **front pair steers**. **Handbrake = drift:** holding it drops rear grip to handbrakeGrip (lower = looser) so the car oversteers and slides; while the tires slip the chassis leans harder and a **looping skid sound** fades in. **Audio:** engineSoundId is a true LOOP whose playback rate rises with speed; skidSoundId loops with volume ∝ slip; brakeSoundId/hornSoundId/collisionSoundId are one-shots (hard brake / H key / hitting something while moving). Wire wheelObjectIds (4 child wheels), steeredWheelIds (front), headlightIds (kind "light" children), and brakeLightIds (emissive children the runtime brightens while braking/handbraking). It uses the same mouse-orbit follow camera (cameraFollow/cameraOffset/cameraPitch/mouseLook). The runtime mirrors the driven car's speed into a project variable named "Speed" (km/h) for a HUD speedometer to bind to. **Lap timing (auto, opt-in):** if the project has a "Lap" variable, the runtime times laps as the driven car passes scene objects named "Checkpoint 0..N" (0 = start/finish) IN ORDER, mirroring "Lap"/"LapTime"/"BestLap" project vars (and a "Checkpoint" cursor) for the HUD and firing the bundled lap_complete/checkpoint chimes — no per-gate blueprint wiring needed. **create_driving_template** builds the whole thing: a "choose your car" start menu (5 bundled cars), a **marked race circuit** (a cone-lined stadium oval with checkpoint gate posts + a start/finish line whose start straight is the grid) on a near-flat streamed terrain world, the full car sound set, headlights, brake lights, and a HUD with a speedometer + lap counter / current & best lap time — selection is gated by a "Driving" project var (0 until a car is chosen). Tune handling/feel/audio with set_vehicle (maxSpeed/acceleration/braking/turnRate/grip/handbrakeGrip/bodyRoll/bodyPitch/suspensionStiffness + the sound ids). The DRIVING LOGIC is an editable blueprint, not hard-coded: the **"Get Drive Input"** node outputs [throttle, steer, handbrake] from the car's keys and the **"Drive"** node feeds that to the Vehicle controller (which does the physics/suspension). A car WITH a blueprint is driven only by its graph (e.g. Update → Branch(Driving>0) → Drive(Get Drive Input)) so users/AI can rewire controls, add boost, gate on a variable, etc.; a car with NO blueprint auto-drives from its keys. create_driving_template ships a shared, editable "Car Controller" blueprint. The **"Get Vehicle Speed"** node reads the current speed (units/sec) for speedometers, gear logic, or speed-gated effects.
- **Combat HUD feedback (auto):** floating **damage numbers** rise from every projectile/melee hit (the struck object itself is NOT tinted — the number is the read); a **hit marker** flashes when the local player's shot lands; a red **hurt flash** vignette pulses when the player takes damage. No wiring — these are driven by the projectile/melee/health system. **Explosives:** give an object an "explosive" instance var (set_object_variable) and on death it BURSTS — a fiery explosion VFX + area damage to every health object within "explosionRadius" (default 4.5) for "explosionDamage" (default 60), which CHAINS to other explosives caught in the blast. Use it for barrels/grenades (the FPS template's barrels are explosive).
...
- **Fastest path — bundled starter game (open-world / Zelda-like):** if the user wants a third-person character/game from scratch (no model imported yet), call create_third_person_template — it builds an **open exploration hub** on a **procedural TERRAIN island** ("The Vale": rolling hills via low heightScale noise, mixed grass+tree foliage, a flattened+painted meadow plaza at the village — a worked example of the terrain sculpt/paint API), under a **golden-hour procedural sky + atmospheric fog** (the scene-environment system). The "Player" pawn (built-in Quaternius rig) carries all four gameplay kits, an over-the-shoulder exploration camera, and a **Fist / Sword / Bow inventory** — the **sword is drawn by default** (LMB melee swing hits a front cone; meleeDamage/meleeRange on the controller), the **bow** (ranged slot) fires arrows gated on RangedMode. HUD = health bar + a **Relics counter** (create_collectible_counter) + a swapping **objective banner**. The hook is a **3-step QUEST CHAIN driven by a village ELDER you talk to (press E — interactable NPC)**: stage 0 → talk → "recover the 3 lost relics" (3 glowing collectibles scattered across the Vale that increment the Relics variable on touch); the Elder's blueprint watches that variable on Update and **auto-advances** to "return to me" once all 3 are in; stage 2 → talk again → "clear the northern ruins & reach the shrine" (heals you); stage 3 → step onto the glowing **shrine** dais (a trigger) → quest complete. A **"Quest Director"** swaps the objective banner each step via Custom Events (q_relics / q_return / q_combat / q_done). The combat encounter sits in the **northern ruins**: two ranged Skeletons (shared editable "Skeleton AI"), a melee Brute, and a tanky **BOSS "Champion"** guarding the shrine — all recolored rigs that ragdoll on death, each with a floating world-space health bar, **aggro-leashed** so they stay dormant in the ruins while you do the early quest steps in the peaceful village. An **intro cinematic** (Director plays a fly-over of the Vale on Start) opens it. Generated assets are foldered (World/Weapons/UI/Player/Quests/Pickups/Enemies/Cinematics). Ready to Play. This is the canonical worked example of an **NPC-driven quest** (interact event → branch on a \`stage\` instance var → Set UI Text dialogue + Fire Event + heal) and of chaining objectives with Custom Event/Fire Event + Show/Hide UI on terrain — follow it to build any quest or open-world starter.
...
- **Projectiles & weapon visibility (combat nodes):** "Spawn Projectile" fires a damaging projectile forward from the owner — a real moving rigid body (with continuous collision detection so fast bullets don't tunnel through thin walls). On its first solid contact it stops; if that hit object has a "health" INSTANCE variable (set one via set_object_variable) it subtracts the damage, then despawns. A wall/prop blocks the shot, so cover works and a foe behind it is never hit. An object whose health hits 0 is destroyed. It is fully configurable on the node (add_node/update_node fields): projectileSpeed, projectileDamage, projectileSize + projectileColor (built-in sphere look), projectileLife (auto-despawn seconds), projectileGravity (0 = straight; raise for an arcing shot), projectileTemplateId (id of a scene object to CLONE as the bullet — its mesh/model/scale/material — for rockets/arrows/orbs; size/color are ignored when set), projectileMuzzle (first-person spawn offset [right, up, forward] from the eye — default down-right where a held gun's barrel is; the shot still converges on the crosshair so it hits where aimed), and projectileDebug (logs every spawn + hit to the runtime console for debugging). In first-person, shots spawn from the weapon muzzle by default (not screen-center). On impact a short particle burst spawns automatically at the hit point. For shot/reload audio, wire a "Play Sound" node (assetId = an imported audio asset) off the fire/reload event. "Set Visible" shows/hides the owner or a Target object during Play (wire a bool into Visible, or set it on the node) — used to equip/holster weapons (attach both to a hand socket, then toggle their visibility). Give enemies a "health" instance variable to make them shootable; bind a world-space health bar to self.health.
- **Enemy AI (node-based, fully editable):** an enemy is just an object with a character controller + a "health" instance variable + a blueprint. Build its brain from these nodes: "Distance To Player" (number → Compare for range checks), "Direction To Player" (normalized vector → wire into Move so it chases), "Player Location" (the player's world position [x,y,z] → wire into Spawn Particle System's location, etc.), "Face Player" (turn to aim), and "Cooldown" (gate: passes once every N seconds — use it before Spawn Projectile for fire rate). "Player" = the active follow-camera character. Recipe for a shooter enemy: Update → Distance To Player → Compare(< attackRange) → Branch → [true] Face Player → Cooldown(1s) → Spawn Projectile; [false] Direction To Player → Move (chase). Player projectiles already damage its health and it dies/ragdolls at 0. Drop enemies via create_object (capsule) → set_character_controller → set_object_variable health → attach_blueprint with the AI graph; or instantiate a saved enemy prefab.
- **First-person view models (arms/weapon):** an object with a viewModel.ownerObjectId renders pinned to that pawn's camera (use cameraMode 'firstPerson'). Its animator AUTO-SOURCES state from the OWNER pawn — Speed/VerticalSpeed/Grounded plus the Aiming(keyAim)/Reloading(keyReload)/Attacking(keyAttack) keys — so building a controller with source 'speed'/'grounded'/'aiming'/'reloading'/'attacking' params makes the arms animate (idle/walk/run/jump/fire/aim/reload) from the player's input with no per-key wiring. Swap weapons by toggling each arm object's Set Visible and firing a manual "Draw" trigger. The bundled FPS template (New FPS) does exactly this with 5 weapon rigs + a 1–5 picker, **hold-to-fire full-auto** (Key Down: Mouse0 → Cooldown gates the fire rate, instead of one shot per click), **explosive barrels**, enemies that are **full-body UAL1 rigs** (recolored, animated) rather than capsules, a **HUD health bar**, walk-over **Health/Ammo pickups**, and a **3-room gated tutorial level**: Room 1 (Controls — move/jump, floating signs + a trigger-opened glowing gate) → Room 2 (Combat — targets + a wall of explosive barrels you shoot through to open the path) → Room 3 (Arena — the enemies + pickups), split by interior divider walls. It also ships a **cinematic layer**: a dark, foggy night-arena environment (update_scene_environment) + bloom/vignette (render settings) so tracers and neon pop, a looping **ambient + music** bed (scene audio), an **intro cinematic** (aerial sweep that settles into the first-person eye, then hands the camera back automatically), **world-space enemy health bars** (a world UI doc bound to self.health / self.maxHealth, attached to each enemy), and a **clear-the-arena win/lose flow** driven by a never-dying **Game Director** empty: it plays the intro on Start, shows a **VICTORY** screen once every enemy is down, and a **YOU DIED** screen when Health hits 0 (each one-shot via a "GameOver" flag). Pattern worth reusing: counting "are all enemies dead" can't be done by the dying enemy (a ragdolled corpse stops ticking) — instead every LIVING enemy re-adds 1 to an "EnemiesAlive" project var each frame, and the Director (which never dies, so it always ticks) snapshots that tally into "AlivePrev" then zeroes it; a frame where the tally stays 0 means all are down. The defeat check watches the engine-mirrored "Health" project var, which stays accurate even though the player ragdolls on death and its own scripts stop. For auto-fire on any gun: gate the held fire key with a Cooldown node. For walking enemies in a first-person scene, import a full-body rig (the FPS kit only has arm rigs) and drive it with a speed-sourced locomotion controller. Gating pattern: a solid "door" prop in a doorway + a trigger volume that Destroys it on player enter, or a shoot-through barrier (explosive barrels). **Proximity tutorial prompts**: a glowing beacon + a trigger zone whose blueprint Shows a hidden screen-UI tip on Trigger Enter (filtered to the player) and Hides it on Trigger Exit — so the tip appears on the HUD only while the player is in range, instead of always-on world text. (Space the zones so they don't overlap.) Polish: give the tip doc a css keyframe + the element an "animation" style to FADE/SLIDE it in, and gate it to show only the FIRST time via a "tipUnseen" instance var on the zone (Get Object Var → Branch → Show + Set Object Var to 0). The FPS starter uses this for its room signs, and both starters' objective/HUD banners share the same fade-in.
...
- Logic: Branch, Compare, AND, OR, Cast.
- Math: Add, Clamp, Lerp.
...
- Prefer high-level/bulk tools over many small calls: create_character_pawn, create_third_person_template, create_first_person_template, create_driving_template, create_ui_template, add_gameplay_kit, spawn_grid, duplicate_object, group_objects, add_ui_preset.
...
- Film Mode cinematics: use create_cinematic/add_cinematic_action for camera cuts, temporary cinematic-only spawns, animation montages, sounds, custom events, visibility, and fades. Camera beats take position+lookAt+fov. For a MOVING camera, prefer a single camera beat with a keyframes array (each keyframe has time, position, lookAt, fov) — the camera flies smoothly through them on a spline; this is the easiest way to author camera animation. For separate static shots that cut, prefer **add_cinematic_shot** (one call per framing — the "shot list" primitive): the first is a hard cut, later ones default to a ~1.2s dolly unless you pass blend:0 for another cut. To animate an OBJECT, prefer a single transform beat with objectId + a transformKeyframes array (each keyframe has time, position, rotation, scale) — it splines through them; this is easier than from/to. Use animate_on_timeline for simple one-shot object moves. Use create_film_mode_template for a worked 3-shot mini-short (cuts + rack focus + a film look).
