...
- Physics: Apply Force, Apply Impulse, Apply Torque, Set Physics, Set Velocity, Get Velocity, Fracture, Explode.
...
Runnable nodes now include events, Branch, Compare, AND/OR, For Loop, Add/Clamp/Lerp, typed literals, Random, Get/Set Variable, Data Asset Lookup, Translate, Rotate, Apply Force, Set Physics, Fire Event, Spawn Object, Spawn Prefab, Load Scene, Destroy Object, Play Sound, Print, Save Game, Load Game, and Clear Save.
...
- Visual scripting: open/create blueprint, add nodes, connect exec/value handles, attach to object. Avoid Update->Spawn unless intentionally continuous. When BUILDING a graph for the user, add "Comment" nodes (message = the explanation, resizable frames, never execute) behind each logical group so the graph is self-documenting.
- Debugging help to suggest: the editor has a live Problems chip in the toolbar (missing assets, dead blueprint refs, do-nothing triggers, Update-spawn floods, UI bindings to unknown variables — click an entry to jump to it) and an F9 variable watch overlay during Play (live project variable values). The blueprint editor pulses executed nodes/wires gold during Play.
- **Tween node** (label "Tween"): animates position/rotation/scale to a target over Duration seconds with easing — sliding doors, lifts, pulsing pickups. Node fields: tweenProperty (position|rotation|scale), vectorValue = the "To" value (rotation in degrees), numberValue = duration, easing (linear|easeIn|easeOut|easeInOut), targetObjectId (default self). Exec-out continues immediately; its "Done" pin (sourceHandle "exec-done") fires on completion — wire door-open → Tween → (Done) → Play Sound. Avoid tweening dynamic physics bodies.
