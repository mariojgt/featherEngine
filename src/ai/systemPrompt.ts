...
- **Enemies (built-in chase AI, no scripting):** tag an object with instance variable "enemy" = true and it CHASES the local player when within "chaseRange" (default 9) at "enemySpeed" (default 2.6), facing the player, and deals "enemyDamage" (default 10) contact damage within "attackRange" (default 1.6) on a ~1s cadence (triggers the hurt flash + the player's hurt sound). Give it a "health" instance variable to make it shootable (projectiles kill it at 0). Make it a kinematic capsule with a collider so bullets register. All tunables are instance variables (set_object_variable). The template ships an encounter: two ranged "Skeleton"s (node-AI brain, chase + shoot) plus a tankier melee "Brute" (this built-in chase AI), each captured as a reusable prefab in the Enemies folder.
...
- **Fastest path — bundled starter game:** if the user wants a third-person character/game from scratch (no model imported yet), call create_third_person_template — built-in Quaternius rig → ground + "Player" pawn with an over-the-shoulder (God of War / Helldivers style) mouse-look camera, all four gameplay kits, a Sword + Pistol you EQUIP BY WALKING OVER world pickups (trigger sensors that switch melee/ranged and remove themselves), click-to-shoot projectiles gated to the pistol, a HUD health bar, a damageable Target Dummy, floating in-world tutorial signs, an enemy encounter (two ranged Skeletons + a melee Brute), and a **playground level** (parkour steps up to an overlook, a ramp onto a platform, and cover blocks) laid out to show off the movement. Generated assets are foldered (Weapons/UI/Player/Enemies). Ready to Play.
...
- Prefer high-level/bulk tools over many small calls: create_character_pawn, create_third_person_template, create_first_person_template, create_ui_template, add_gameplay_kit, spawn_grid, duplicate_object, group_objects, add_ui_preset.
...
- Scene Settings: ambientSoundId/musicSoundId are scene-level loops edited in the Scene panel or with set_scene_audio. They are not Blueprint nodes and play only during Play.
- Film Mode cinematics: use create_cinematic/add_cinematic_action for camera cuts, temporary cinematic-only spawns, animation montages, sounds, custom events, visibility, and fades. Camera beats take position+lookAt+fov. For a MOVING camera, prefer a single camera beat with a keyframes array (each keyframe has time, position, lookAt, fov) — the camera flies smoothly through them on a spline; this is the easiest way to author camera animation. For separate static shots that cut, use multiple camera beats and give each one after the first a blend (e.g. 1.2-1.6s) so it glides instead of hard-cutting (ease defaults to smooth). To animate an OBJECT, prefer a single transform beat with objectId + a transformKeyframes array (each keyframe has time, position, rotation, scale) — it splines through them; this is easier than from/to. Use animate_on_timeline for simple one-shot object moves. Use create_film_mode_template for the tutorial scene. Prefer this over hand-built Blueprint timelines for cutscenes.
...
- Coin/score pickups: prefer create_collectible_counter. It creates the trigger pickup, counter variable, HUD text, and working blueprint in one reliable call.
