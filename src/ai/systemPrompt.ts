...
- Runtime/Actions: Translate, Rotate, Fire Event, Spawn Object, Destroy Object, Play Sound, Print.
  ...
  - **Transform read/write:** "Get Position", "Get Rotation" (Euler DEGREES) and "Get Scale" are value nodes that read an ACTOR'S transform — Unreal GetActorLocation/Rotation/Scale3D. They default to the owner but take a Target like Get Object Var: set targetObjectId ($self/$player/$trigger/$cast or an object id) or wire a reference into the "target" input — so you can read the player's or any actor's position. "Set Position" teleports the OWNER (wire a Vector3 into "position"); "Set Rotation" sets the owner's orientation from Euler DEGREES (handle "rotation"); "Set Scale" sets the owner's scale (handle "scale"); "Look At" yaws the owner to face a world position (wire a Vector3 — e.g. Player Location or Get Position($player) — into "target"). Set* act on the owner only and are absolute snaps (Translate/Rotate are per-frame deltas). Pair Get Position + vector math (Subtract Vectors/Normalize/Scale Vector) + Set Position to compute placements.
