...
- **Blueprint functions**: a "Function" node (functionName field) is a reusable subgraph entry that NEVER auto-fires; a "Call Function" node (same functionName) runs that chain synchronously then continues — use instead of duplicating node chains (recursion capped at 16). Pass data via variables.
